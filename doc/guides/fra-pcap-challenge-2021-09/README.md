...
*       │until 0x4fa1ff.7 (1024)                        │                │
0x4fa200│00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00│................│  unknown0: raw bits
*       │until 0x4fafff.7 (end) (3584)                  │                │
```
...
`split("\n")` splits a string on some boundary, `"\n"` in this case, and output an array with all the parts between. We then use `.[0,1,-2,-1]` to get the first, second, second from the end and last string from the array (negative indexes counts from the end).

Here we use the output operator `,` which is a way in jq to "output" more than one value. What happens is that for each value `0`,`1`, `-2` and `-1` the program will fork and continue to run and then at some later point it will backtrack back to the next value and so on. To learn more one can read all about it on
...
The common `3c652e23657d67` prefix is suspicious, looks hex encoded and probably some encrypted form of `{"id": ` judGing by how other "update" messages look. There was "long_xor" so maybe this is just per byte xor? XOR encryption is symmetric so we should get the key if we encrypt some known plaintext with it's encrypted version.
...
`explode` is jq functions that turn a string into an array of its codepoints. `hex` decodes a hex string into a bytes buffer but we use it as string here. We then bitwise XOR using `bxor` and get the key `71`.

Notice that jq uses the argument separator `;` not `,`.

TODO: rewrite once buffers support explode etc, codepoints works here because the string uses only ASCII.
...
Ok that looks like JSON to me. Let's figure out "long_xor", "msg_type" is "code" so maybe it starts with `class `?.
