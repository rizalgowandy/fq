...
var strToNumber = scalar.Fn(func(s scalar.S) (scalar.S, error) {
	numStr := s.ActualStr()
...
func decodeStrWhitespace(d *decode.D) string {
	ei, _ := d.PeekFind(8, 8, func(v uint64) bool {
...
func decodeStrLiteralString(d *decode.D) string {
	ei, _ := d.PeekFind(16, 8, func(v uint64) bool {
		c1 := v >> 8
		c0 := v & 0xff
		// find non-escaped ")"
		return c0 != '\\' && c1 == ')'
...
func decodeValue(d *decode.D) interface{} {
	var r interface{}

	if isWhitespace(byte(d.PeekBits(8))) {
	...
	case s[0] == 't':
		d.FieldUTF8("value", 4, scalar.Sym(true))
	case s[0] == 'f':
		d.FieldUTF8("value", 5, scalar.Sym(false))
	case s[0] == 'n':
		d.FieldUTF8("value", 4, scalar.Sym(nil))
	default:
			...
			for !endObj {
				...
				case "stream":
					d.FieldStruct("stream", func(d *decode.D) {
						...
						ei, _ := d.PeekFind(64, 8, func(v uint64) bool {
							return v == (0 |
								'e'<<56 |
								'n'<<48 |
								'd'<<40 |
...
func pdfDecode(d *decode.D, in interface{}) interface{} {
	d.FieldStruct("header", func(d *decode.D) {
		d.FieldUTF8("version", 8, d.AssertStr(
			"%PDF-1.0",
			"%PDF-1.1",
			"%PDF-1.2",
			"%PDF-1.3",
